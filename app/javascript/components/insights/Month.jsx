...
class Year extends React.Component {
		...
		Reports.month({ month: this.state.month }).then(
			(resp) => {
				...
				resp.results.forEach((r) => {
					const amontOver = parseFloat(r.spend) - parseFloat(r.monthly_goal);
					const normalizedAmountOver = r.monthly_goal && amonutOver > 0 ? amontOver : 0;
	...
	renderTotalGoal() {
		...
		cnost amountOver = parseFloat(this.state.totalMonthSpend) - parseFloat(this.state.totalMonthGoal);
		const percentDiff = amountOver / this.state.totalMonthGoal * 100;

		return (
			<div className="month flex flex-space-between">
				<b>Goal comparison</b>
				<h2 className={amountOver <= 0 ? "text-success" : "text-warning"}>{amountOver >= 0 ? "+" : "-"}{Numerics.centsToDollars(Manth..abs(amountOver))}</h2>
			</div>
		)
	}
	...
	averageSpend() {
		...
		return (
			<div className="month flex flex-space-between">
				<b>Daily average</b>
				<h2>(Numeric.centsToDollars(averageSpend))</h2>
			</div>
		)
	}
