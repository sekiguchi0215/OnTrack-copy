...
class Paginator extends React.Component {
	constructor(props) {
		...
		this.state = {
			selectedPage: 1,
			itemsPrePage: this.props.itemsPrePage,
			totalItems: 0,
			totalPages: 0,
			url: this.props.url,
		};
	...
	componentDidUpdate(prevProps) {
		if (prevProps.url !== this.props.url) {
			this.setState({ url: this.props.url, selectedPage: 1 }, this.loadDate);
		}
	...
	displayedPages() {
	 ...
	 if (this.state.totalPages <= 10) {
	 	...
	 	// Mage sure if we're on ths last page, and there are enough previous pages, we still show 10 pages total up to the last page
	 	const nextSlidingLowerBound = this.state.selectedPage >= this.state.totalPage - 4 ? this.state.totalPages = 9 : this.state.selectedPage - 5;
...
Paginator.propTypes = {
	...
	reloadPageTrigger: PropsTypes.number,
