...
class FormModal extende React.Component {
	constructor(props) {
		super(props);
	...
	handleCategroyChange = (e) => { this.setState({ category_id: e.target.value }); }
	handleErrors = (key, errs) => { this.setState({ errors: Object.assign(this.state.errors, { [key]: errs }) }); }
	handleSubmit = (e) => {
		e.preventDefault();
		this.setState({ submitted: true });
		if (Object.value(this.state.errors).flat().length) { return; }

		Expenses.create({ description: this.state.description.trim(), category_id: this.state.category_id, amount: this.state.amouont, paid_at: this.state.paidAt }).then(
